...
extension FirestoreReadAndWritable {
    /// Accesses FireStore database with user's uid stored in defaults and **removes** the product's id from corresponding **array** that is stored in firestore db.
    /// - parameters:
    ...
    /// - warning: This function's collection is **"users"** and only adds or removes the id parameter to the **array** in Firebase.
    ///
    func addProductTo(documentPath: FirestoreDocumentPath.RawValue, withId id: Int?, completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    ///     - completion:: To handle possible outcomes.
    /// - warning: This function's collection is **"users"** and only adds or removes the id parameter to the **array** in Firebase.
    func removeProductFrom(documentPath: FirestoreDocumentPath.RawValue, withId id: Int?, completion: @escaping ((_ error: Error?) -> Void)) {
        guard let id = id,
              let uid = uid else { return }
...
// TODO: Maybe some properties can be defined in protocol ( like "users" ) and a data type as a type of enum for the document type like array, int, string and returns the related function to user.
